...
# Reorder the category_selected vector based on sorted_categories
...
min.ratio <- as.numeric(min(d1$data$GeneRatio, na.rm = TRUE))
...
d1 +
  ...
  # scale x part was conducted manually
  scale_x_continuous(limits = c(0, max.ratio+0.005),
                     breaks = c(0.05, 0.10),
                     labels = c("0.05", "0.10")) +
  ## without 'scale_y_discrete()', categorys will be ordered as its values calculated by enrichGO
  scale_y_discrete(
    limits = rev(levels(category_selected)),
    ## line breaking custom by using "str_wrap"
...
# group for generation of orders
...
if(max.value > 0 & min.value >= 0){
  ...
  ## In this project I don't need the color below
